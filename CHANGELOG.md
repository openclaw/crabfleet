...
- Require every direct Share This Mac tailnet listener to authenticate with a per-run in-memory 12-character password through ARD or VNC DES, add source-IP backoff and lockout, native Keychain and browser sessionStorage credential flows, and an explicit relay-only RFB authentication bypass while leaving the owner-authenticated Worker relay wire unchanged.
