...
- Fence Share This Mac registry cleanup with per-registration ownership tokens and return the exact atomically written registration row so delayed or overlapping publishers cannot displace cleanup authority, while retaining owner-authenticated tokenless cleanup for migrated legacy registrations only.
