...
- Fit the native Mac VNC desktop flush to the available window, keep its remote resolution synchronized to the viewport and current display pixel density when supported, compact the surrounding controls, and use an available stable signing identity for local app bundles so Keychain access survives rebuilds.
