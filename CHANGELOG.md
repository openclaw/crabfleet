...
- Add capability-negotiated `CFR1` input, output, and acknowledgement frames for GitHub Actions runners, retain legacy raw runner compatibility, prevent binary PTY output from colliding with relay control frames, and keep multiplex dispatch responsive while framed input acknowledgements are pending.
