...
- **Share This Mac.** Stream up to four Mac displays privately over RFB-authenticated, SPKI-pinned QUIC-first Tailscale endpoints with TCP fallback and owner-authenticated per-display browser relays, using a fresh copyable 12-character password per share run, four concurrent viewers per display, low-latency negotiated client-side cursors, feature-probed browser and native full-chroma HEVC plus H.264 and Tight/JPEG fallback, browser and native primary-display AAC-LC audio, multicast clipboard, an optional single shared folder with native/browser downloads and uploads, dirty-rect idle suppression, per-viewer Auto/Sharp/Smooth quality controls, throughput-aware rate control, resize, transport and quality diagnostics, and live view-only and audio controls.
- **Share This Mac.** Stream up to four Mac displays privately over RFB-authenticated, SPKI-pinned QUIC-first Tailscale endpoints with TCP fallback and owner-authenticated per-display browser relays, using a fresh copyable 12-character password per share run, four concurrent viewers per display, low-latency negotiated client-side cursors, feature-probed browser and native full-chroma HEVC plus H.264 and Tight/JPEG fallback, browser and native primary-display AAC-LC audio, multicast clipboard, dirty-rect idle suppression, per-viewer Auto/Sharp/Smooth quality controls, throughput-aware rate control, resize, transport and quality diagnostics, live view-only and audio controls, and a hidden Remote Desktop-permission experiment for macOS capture-indicator research.
- **Share This Mac.** Stream up to four Mac displays privately over RFB-authenticated, SPKI-pinned QUIC-first Tailscale endpoints with TCP fallback and owner-authenticated per-display browser relays, using a fresh copyable 12-character password per share run, four concurrent viewers per display, low-latency negotiated client-side cursors, feature-probed browser and native full-chroma HEVC plus H.264 and Tight/JPEG fallback, browser and native primary-display AAC-LC audio, multicast clipboard, dirty-rect idle suppression, per-viewer Auto/Sharp/Smooth quality controls, throughput-aware rate control, resize, transport and quality diagnostics, and live view-only and audio controls.
- **Wake saved desktops.** Add a MAC address to a native macOS saved connection, send a Wake-on-LAN packet from its card, or opt into one wake-and-retry after the initial TCP connection fails.
