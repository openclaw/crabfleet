...
- **Share This Mac.** Stream up to four Mac displays privately over separate Tailscale endpoints and owner-authenticated per-display browser relays, with four concurrent viewers per display, full-chroma hardware HEVC for capable native viewers, hardware H.264 and Tight/JPEG fallback, primary-display AAC-LC audio, multicast clipboard, dirty-rect idle suppression, live Auto/Sharp/Smooth quality modes, throughput-aware rate control, resize, and live view-only and audio controls.
